# Дадено е податочно множество со податоци за квалитет на вино. Податочното множетво е дефинирано во променливата dataset_script. Последната колона ја содржи оценката за квалитетот на виното која може да има вредности помеѓу 0 и 10. Сите колони содржат податоци од непрекинат тип.
#
...
if __name__ == '__main__':
    ...
    for row in dataset:
        new_row = row
        if row[-1] < 5:
            class_0.append(new_row)
        elif row[-1] >= 5:
            class_1.append(new_row)
    ...
    train_0 = class_0[int(len(class_0) * x/100):]
    test_0 = class_0[:int(len(class_0) * x/100)]
    train_1 = class_1[int(len(class_1) * x/100):]
    test_1 = class_1[:int(len(class_1) * x/100)]
    ...
    attributes = list(classifier_tree.feature_importances_)

    least_important_feature = attributes.index(min(attributes))
    ...
    encoder1.fit([row[:-1] for row in new_dataset])
    class_0 = []
    class_1 = []
    for row in dataset:
        new_row = row
    ...
    train_x = encoder1.transform(train_x)
    test_x = encoder1.transform(test_x)

    classifier_neuron = MLPClassifier(hidden_layer_sizes=15, activation='relu', learning_rate_init=0.001, max_iter=200,
                                      random_state=0)

    standard = StandardScaler()
    standard.fit(train_x)
    standard.transform(train_x)
    classifier_neuron.fit(train_x, train_y)
    print("Tochnost so StandardScaler:", classifier_neuron.score(train_x, train_y))
    minmax = MinMaxScaler()
    minmax.fit(train_x)
    minmax.transform(train_x)
    classifier_neuron.fit(train_x, train_y)
    print("Tochnost so MinMaxScaler:", classifier_neuron.score(train_x, train_y))
