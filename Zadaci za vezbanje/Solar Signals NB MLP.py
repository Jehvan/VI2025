...
if __name__ == '__main__':
    ...
    if model == "NB":
        ...
        classifier_1 = MLPClassifier(hidden_layer_sizes=(50,), activation="relu",learning_rate_init=0.001)
        classifier_2 = MLPClassifier(hidden_layer_sizes=(50,), activation="relu",learning_rate_init=0.001)
        classifier_3 = MLPClassifier(hidden_layer_sizes=(50,), activation="relu", learning_rate_init=0.001)
        classifier_4 = MLPClassifier(hidden_layer_sizes=(50,), activation="relu", learning_rate_init=0.001)


    class_0 = [row for row in data if row[-1] == 0]
    class_1 = [row for row in data if row[-1] == 1]

    subset_1 = class_0[:int(len(class_0) * 0.25)] + class_1[:int(len(class_1) * 0.25)]
    subset_2 = class_0[int(len(class_0) * 0.25):int(len(class_0) * 0.5)] + class_1[int(len(class_1) * 0.25):int(len(class_1) * 0.5)]
    subset_3 = class_0[int(len(class_0) * 0.5):int(len(class_0) * 0.75)] + class_1[int(len(class_1) * 0.5):int(
        len(class_1) * 0.75)]
    subset_4 = class_0[int(len(class_0) * 0.75):] + class_1[int(len(class_1) * 0.75):]
    sum = 0
    accuracy_1 = 0.0
    accuracy_2 = 0.0
    accuracy_3 = 0.0
    ...
    train_set_1 = subset_2 + subset_3 + subset_4
    test_set_1 = subset_1

    train_x = [row[:-1] for row in train_set_1]
    train_y = [row[-1] for row in train_set_1]
    test_x = [row[:-1] for row in test_set_1]
    test_y = [row[-1] for row in test_set_1]

    classifier_1.fit(train_x, train_y)
    accuracy_1 = classifier_1.score(train_x, train_y)

    train_set_2 = subset_1 + subset_3 + subset_4
    test_set_2 = subset_2

    train_x = [row[:-1] for row in train_set_2]
    train_y = [row[-1] for row in train_set_2]
    test_x = [row[:-1] for row in test_set_2]
    test_y = [row[-1] for row in test_set_2]

    classifier_2.fit(train_x, train_y)
    accuracy_2 = classifier_2.score(train_x, train_y)

    train_set_3 = subset_1 + subset_2 + subset_4
    test_set_3 = subset_3

    train_x = [row[:-1] for row in train_set_3]
    train_y = [row[-1] for row in train_set_3]
    test_x = [row[:-1] for row in test_set_3]
    test_y = [row[-1] for row in test_set_3]

    classifier_3.fit(train_x, train_y)
    accuracy_3 = classifier_3.score(train_x, train_y)

    train_set_4 = subset_1 + subset_2 + subset_3
    test_set_4 = subset_4

    train_x = [row[:-1] for row in train_set_4]
    train_y = [row[-1] for row in train_set_4]
    test_x = [row[:-1] for row in test_set_4]
    test_y = [row[-1] for row in test_set_4]

    classifier_4.fit(train_x, train_y)
    accuracy_4 = classifier_4.score(train_x, train_y)

    sum = accuracy_1 + accuracy_2 + accuracy_3 + accuracy_4
    print("Prosechna tochnost:",sum/4)
    ...
    subset_1 = class_0[:int(len(class_0) * 0.25)] + class_1[:int(len(class_1) * 0.25)]
    subset_2 = class_0[int(len(class_0) * 0.25):int(len(class_0) * 0.5)] + class_1[int(len(class_1) * 0.25):int(
        len(class_1) * 0.5)]
    subset_3 = class_0[int(len(class_0) * 0.5):int(len(class_0) * 0.75)] + class_1[int(len(class_1) * 0.5):int(
        len(class_1) * 0.75)]
    subset_4 = class_0[int(len(class_0) * 0.75):] + class_1[int(len(class_1) * 0.75):]
    ...
    sum = accuracy_1 + accuracy_2 + accuracy_3 + accuracy_4
    print("Prosechna tochnost so otstraneta kolona:", sum / 4)