...
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        ghost = state
        if ghost not in walls and 0 <= ghost[0] < n and 0 <= ghost[1] < n:
            return True
        return False
    ...
    def successor(self, state):
        ...
        for move in moves:
            ...
            newx, newy = ghost[0] + direction[0], ghost[1] + direction[1]
            if self.check_valid((newx, newy), self.walls, self.n):
                successors[move] = (newx, newy)

        return successors

    def h(self, node):
        state = node.state
        distance = math.floor(abs(state[0] - self.goal[0]) + abs(state[1] - self.goal[1]))
        return distance/3
