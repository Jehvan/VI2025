...
class Squares(Problem):
    ...
    def successor(self, state):
        successors = dict()
        kvadratcinja = list(state)

        moves = {
            "Pomesti kvadratche X gore":(0,1),
            "Pomesti kvadratche X levo":(-1,0),
            "Pomesti kvadratche X desno":(1,0),
            "Pomesti kvadratche X dolu":(0,-1),
        }

        for i in range(5):
            new_kvadratcinja = kvadratcinja[:]
            kvadratce = new_kvadratcinja[i]
            for move in moves:
                direction = moves[move]
                new_move = move.replace("X",str(i + 1))
                newx, newy = kvadratce[0] + direction[0], kvadratce[1] + direction[1]
                new_kvadratce = (newx, newy)
                new_kvadratcinja[i] = new_kvadratce
                if self.check_valid(new_kvadratcinja):
                    new_state = tuple(map(tuple,new_kvadratcinja))
                    successors[new_move] = new_state
        return successors
