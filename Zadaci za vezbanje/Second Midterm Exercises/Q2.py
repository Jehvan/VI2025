...
if __name__ == '__main__':
    ...
    classifier_tree = DecisionTreeClassifier(random_state=0,criterion='entropy')
    classifier_forest = RandomForestClassifier(n_estimators=4,random_state=0,criterion='entropy')
    classifier_neuron = MLPClassifier(hidden_layer_sizes=(10,),activation='relu',learning_rate_init=0.001,random_state=0)
    ...
    train_set = class_0[:int(len(class_0) * p)] + class_1[:int(len(class_1) * p)]
    test_set = class_0[int(len(class_0) * p):] + class_1[int(len(class_1) * p):]
    ...
    classifier_neuron.fit(train_x, train_y)

    NB_score = classifier_nb.score(test_x,test_y)
    Tree_score = classifier_tree.score(test_x,test_y)
    Forest_score = classifier_forest.score(test_x,test_y)
    Neuron_score = classifier_neuron.score(test_x,test_y)
    najtocen_classifier = ""


    if NB_score > Tree_score and NB_score >= Forest_score and NB_score >= Neuron_score:
        print("Najgolema tocnost ima klasifikatorot Naive Bayes")
        najtocen_classifier = 0
    elif Tree_score >= Forest_score and Tree_score >= Neuron_score:
        print("Najgolema tocnost ima klasifikatorot Decision Tree")
        najtocen_classifier = 1
    elif Forest_score >= Neuron_score:
        print("Najgolema tocnost ima klasifikatorot Random Forest")
        najtocen_classifier = 2
    else:
        print("Najgolema tocnost ima klasifikatorot MLP")
        najtocen_classifier = 3
    ...
    for i in range(len(test_set)):
        ...
        vrednost_1 = 0
        true_class = test_y[i]
        prediction_NB = classifier_nb.predict([test_x[i]])[0]
        prediction_tree = classifier_tree.predict([test_x[i]])[0]
        prediction_forest = classifier_forest.predict([test_x[i]])[0]
        prediction_neuron = classifier_neuron.predict([test_x[i]])[0]
        predictions = [prediction_NB, prediction_tree, prediction_forest, prediction_neuron]
        for j in range(0,4):
            if predictions[j] == 0:
                if j == najtocen_classifier:
                    vrednost_0 = vrednost_0 + 2
                else:
                    vrednost_0 = vrednost_0 + 1
            elif predictions[j]== 1:
                if j == najtocen_classifier:
                    vrednost_1 = vrednost_1 + 2
                else:
                    vrednost_1 = vrednost_1 + 1

        if vrednost_0 > vrednost_1:
            score = 0
        else:
            score = 1
        if score == true_class and score == 1:
            TP += 1
        elif score != true_class and score == 0:
            FN += 1
    print("Odzivot na kolekcijata so klasifikatori e", (TP / (TP + FN)))